...
import imagehash
import random  # for math CAPTCHA
import smtplib
...
# secret key for session (demo)
app.secret_key = "change-this-secret-key"

# Simple admin credentials (demo ke liye)
...
# pHash duplicate sensitivity (cropped / similar images ke liye thoda relax)
PHASH_DUP_THRESHOLD = 10        # smaller = strict, bigger = zyada sensitive

# ---------- REAL EMAIL NOTIFICATION CONFIG (GMAIL SMTP) ----------
EMAIL_ENABLED = True

SMTP_HOST = "smtp.gmail.com"
SMTP_PORT = 587
SMTP_USER = "unmakedengineer@gmail.com"
# App Password (spaces hata ke)
...
def send_admin_alert(event_type: str, message: str, extra: dict | None = None):
    ...
    # diff jaisi values kabhi-kabhi numpy.int64 ho sakti hain -> int/str me cast
    safe_extra = {}
    if extra:
    ...
    if EMAIL_ENABLED:
        try:
            ...
            # Agar mapping naa mile to event_type ko thoda human readable bana do
            ...
            # Clean extra details instead of raw JSON
            if safe_extra:
...
def record_honey_hit(listing_id, ip, ua, referer):
    ...
    try:
        ...
        # ADMIN notification for every honey hit (low severity)
        send_admin_alert(
            "honey_hit",
...
# `/admin/login` ko bhi support karo (404 fix)
...
def listing_detail(listing_id):
    ...
    # SIMPLE MATH CAPTCHA QUESTION (per-page)
...
# ---------- CONTACT FORM (with CAPTCHA check) ----------
...
def contact_owner(listing_id):
    ...
    if expected is None or user_answer_int != expected:
        # Wrong captcha -> same listing page w/ error
        listing = fetch_listing_by_id(listing_id)
        if not listing:
            return "Listing not found", 404
